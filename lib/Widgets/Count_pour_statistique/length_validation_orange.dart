...
class NotValidation extends StatefulWidget {
  const NotValidation({super.key});

  @override
  State<NotValidation> createState() => _NotValidationState();
}

class _NotValidationState extends State<NotValidation> {
  ...
  Future<int> getCount() async {
    ...
    for (var datax in mongoModels) {
      if (datax.statutRealise == 'TERMINEE_OK' && datax.forfait.valide == false) {
        count++;
      }
    }
    return count;
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Row(
            children: [
              Column(
                children: [
                  Icon(Icons.check_box, size: 30, color: Colors.orange),
                  Text(
                    "${snapshot.data}",
                    style: TextStyle(
