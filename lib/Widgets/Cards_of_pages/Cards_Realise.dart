...
class MyCardRealise extends StatefulWidget {
  const MyCardRealise({super.key});

  @override
  State<MyCardRealise> createState() => _MyCardRealiseState();
}

class _MyCardRealiseState extends State<MyCardRealise> {
  @override
  ...
  @override
    Widget displayCardRealise(MongoDbModel data){
    ...
    List<String> codesInterdits = ['11', '3', '17', '18', '48', 'INR1a', 'INR1b', 'INR1c'];
      ...
      if(data.logement!.contains('IMMEUBLE') ){
        logementIcon = Icons.apartment_outlined;
      }else if(data.logement!.contains('ASSIST') ){
        logementIcon = Icons.person_add_alt_1;
      }else if(data.logement!.contains('PAVILLON')){
        logementIcon = Icons.home;
      }else if(data.logement!.contains('NACELLE')) {
        logementIcon = Icons.upload_sharp;
      }else if(data.logement!.contains('PLP')){
      ...
      if (data.statutRealise == 'TERMINEE_OK') {
        ...
        etatRealiseIcon = Icons.dangerous;
      ...
      if(data.diagnosticWifi == null || data.diagnosticWifi.diagnosticWifiInclus == false){
        wifi = null;
      }else{
        wifi = Icons.wifi;
      }
      ...
      if(data.typeIstallation == "AVEC" &&
        ...
        if(data.validationCR.typeInstallationDeclareTechnicien!.startsWith("E1")){
          declaration= "P";
        }else if (data.validationCR.typeInstallationDeclareTechnicien!.startsWith("O") ||data.validationCR.typeInstallationDeclareTechnicien!.startsWith("E2")){
        ...
        child: Padding(
          ...
          child: Stack(
            children: [
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      Row(children:[
                        ...
                        Icon(nbvoisinIcon,
                            size: 23,
                            color: data.nbVoisin! > 0 || data.nbVoisinGrades! > 0? Colors.red:Colors.green,  ),
                        SizedBox(width: 5,),
                        Icon(userQuestionIcon,
                          size: 23,
                          color: data.statutRealise == 'TERMINEE_OK' && data.validationCR.incoherent == false?
                            Colors.red : Colors.green ,
                        ),
                        SizedBox(width: 5,),
                        Icon(wifi,
                          size: 23,
                          color: data.diagnosticWifi != null && data.diagnosticWifi.diagnosticWifiInclus == true && data.diagnosticWifi.diagnosticWifiRealise == false?
                              Colors.red : Colors.green,
                        ),
                        SizedBox(width: 5,),
                        Icon(photoManquantesIcon,
                          size: 23,
                          color: data.photoManquantes != null && data.photoManquantes!.length > 0? Colors.red: Colors.green,
                        ),
                         Icon(geo_photoIcon,size: 23,color: data.images.contains('Photo non géolocalisée')?Colors.red: Colors.green),
                        SizedBox(width: 5,),
                        Icon(phoneIcon,
                          size: 23,
              ...
              Positioned(
                child: Text("${data.debutIntervention != null ? timeFormat.format(data.debutIntervention!) : ''} - ${data.finIntervention != null ? timeFormat.format(data.finIntervention!) : ''}",
  ...
  Widget buildRow1(String text1,IconData icon1,String text2,IconData icon2,IconData icon3, String text3) {
    return Row(
      children: [
        Text(text1,style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold),),
        SizedBox(width: 50),
        Icon(icon1),
        Text(text2,style: TextStyle(fontSize: 17,fontWeight: FontWeight.w500),),
        SizedBox(width: 5),
        Icon(icon2),
        SizedBox(width: 5),
        Icon(icon3),
        SizedBox(width: 5),
        Text(text3,style: TextStyle(fontSize: 16,fontWeight: FontWeight.w400),),
      ],
    );
  }
